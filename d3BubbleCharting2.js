        ...
        if(issueCount>0){
            fullLabelList.forEach(t=>{
                if(t != "projectId" && t != "totalIssue" && t != "name"){
					var realValue=dataItem[t];
					//calculate percentage
                    dataItem[t]=(dataItem[t]/issueCount*100);
                }
            })
        }                
        ...
        if(t != "projectId" && t != "totalIssue" && t !="name"){
            data4Histogram.forEach(p=>{
                ...
                data4HistogramChart.push({
                    "label":label, "quantity":quantity, "projectId":projectId, 'totalIssue':p['totalIssue']
                })
            })
        }
    })
        ...
        if(t != "projectId" && t != "totalIssue" && t != "name"){
            ...
            data4HistogramChart.forEach(p=>{
                if(t==p.label){
                    ...
                    dataItem[columns[p.projectId+1]]={totalIssue: p.totalIssue , issues:p.quantity==0?1:p.quantity };
                }
            })
    ...
    g.append("g")
        ...
        .data(function(d) { return keys.map(function(key) { 
			return {
				key: key, 
				value: d[key].issues,
				totalIssue:d[key].totalIssue}; 
				}); })
        .enter().append("rect")
        .attr("x", function(d) { return x1(d.key); })
        .attr("y", function(d) { a = y(d.value);
					console.log(d.value + " y() " + a);
					 a =  (100-d.value)/100*height;
					 return a;})
        .attr("width", x1.bandwidth())
		// y() does return NaN rather than what it should.
        .attr("height", function(d) { a =  (100-d.value)/100*height; return height - a; })
        ...
        .on("mousemove", function(d){
            tooltip
              ...
              .html((d.key) + "<br>" + Math.floor(d.totalIssue * d.value / 100));
        })
