...
var makeBubbleChart = function(data) {
    ...
    var assigneeOpenIssuesDistinct= new Set(allOpenIssues.map(x=>x.assignee!= null? x.assignee.login: "not assigned"))
    , assigneeAddressedOpenDistinct= new Set(addressedOpen.map(x=>x.assignee!= null? x.assignee.login:"not assigned"))
    , assigneeBugIssuesDistinct= new Set(allBugIssues.map(x=>x.assignee!= null? x.assignee.login: "not assigned"))
    , assigneeAddressedBugDistinct= new Set(addressedBug.map(x=>x.assignee!= null? x.assignee.login: "not assigned"))
    , assigneeEnhancementIssuesDistinct= new Set(allEnhancementIssues.map(x=>x.assignee!= null? x.assignee.login: "not assigned"))
    , assigneeAddressedEnhancementDistinct= new Set(addressedEnhancement.map(x=>x.assignee!= null? x.assignee.login: "not assigned"));
    ...
    function chart(selection) {
        ...
        var node = svg.selectAll("circle").data(data).enter().append("circle")
            ...
            .on("mouseover", function(d) {
                tooltip.html(d[columnForColors] + "<br>" + d.title + "<br>" + d[columnForRadius] + " issues");
